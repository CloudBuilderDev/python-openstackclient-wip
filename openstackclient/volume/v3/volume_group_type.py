...
class SetVolumeGroupType(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.no_property:
            try:
                specs = volume_client.get_group_type_specs(group_type)
...
class ListVolumeGroupType(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        if filters and not sdk_utils.supports_microversion(
            volume_client, "3.33"
        ):
            raise exceptions.CommandError(
        ...
        # TODO(happy7656): Should ListVolumeGroupType also display 'volume_types'?
        column_headers = (
            'ID',
            'Name',
            'Is Public',
